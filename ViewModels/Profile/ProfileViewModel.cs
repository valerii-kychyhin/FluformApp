...
public partial class ProfileViewModel : ObservableObject
{
    ...
    private async void OpenWorkflows()
    {
        try
        {
            // Получаем WorkflowViewModel через DI
            var workflowViewModel = _services.GetRequiredService<WorkflowViewModel>();

            // Создаем страницу через конструктор, передавая ViewModel
            var workflowPage = new WorkflowPage(workflowViewModel);

            // Проверяем, что MainPage это NavigationPage
            if (Application.Current.MainPage is NavigationPage navPage)
            {
                // Пушим страницу в стек навигации
                await navPage.PushAsync(workflowPage);
            }
            else
            {
                // Если MainPage не NavigationPage, оборачиваем WorkflowPage в NavigationPage
                Application.Current.MainPage = new NavigationPage(workflowPage);
            }

            // Загружаем user workflows (для проверки работы)
        ...
        catch (Exception ex)
        {
            await Application.Current.MainPage.DisplayAlert(
                "Error",
                $"Failed to open workflows: {ex.Message}",
                "OK"
            );
        }
    }
